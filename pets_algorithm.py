...
class ReplayBuffer:
    ...
    def return_all_samples(self):
        ...
        # 确保状态数据是正确的一维数组，处理不同形状的状态
        state_list = []
        next_state_list = []
        for s in state:
            # 处理状态可能是元组的情况（如 (array, {})）
        ...
        for ns in next_state:
            ...
            ns_array = np.array(ns, dtype=np.float32)
            if ns_array.ndim > 1:
                ns_array = ns_array.flatten()
            next_state_list.append(ns_array)
...
class PETS:
    ...
    def train_model(self):
        ...
        obs = env_samples[0]
        actions = env_samples[1]
        rewards = env_samples[2].reshape(-1, 1)
...
def main():
    ...
    plt.plot(episodes_list, return_list, 'b-o', linewidth=2, markersize=6)
    plt.xlabel('Episodes', fontsize=12)
    plt.ylabel('Returns', fontsize=12)
    plt.title('PETS Algorithm Performance on {}'.format(env_name), fontsize=14)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
    
    print("训练完成！")
    print(f"最终回报: {return_list[-1]}")
